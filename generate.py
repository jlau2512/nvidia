...
def generate_video(image_path: str, output_path: str = "output_video.mp4") -> str:
    """Animate an image into video using Wan2.1 image-to-video."""
    ...
    url = f"{BASE_URL}/genai/wan/wan2.1-img2video"
    payload = {
    ...
    if response.status_code == 202:
        ...
        video_bytes = _poll_for_result(request_id, url)
    elif response.status_code == 200:
        data = response.json()
        video_bytes = _extract_video(data)
    else:
        print(f"ERROR {response.status_code}: {response.text}")
        sys.exit(1)
...
def _extract_video(data: dict) -> bytes:
    artifacts = data.get("artifacts", [])
    if not artifacts:
...
def _poll_for_result(request_id: str, url: str, max_wait: int = 300) -> bytes:
    ...
    while waited < max_wait:
        time.sleep(interval)
        waited += interval
        resp = requests.get(url, headers=poll_headers, timeout=60)
        ...
        if resp.status_code == 200:
            return _extract_video(resp.json())
